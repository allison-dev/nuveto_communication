...
if (!function_exists('sendFivenine')) {
    function sendFivenine($id, $message = '', $channel = 'chat', $method = 'post', $sendUrl = '/messages', $parameter = false, $external_auth_id = false)
    {
        ...
        if ($channel == 'twitter') {
            ...
            $data = DB::table('chat_conversations')->where('clientId', '=', $id)->orderBy('id', 'desc')->first();
...
if (!function_exists('sendMessageWhatsapp')) {
    function sendMessageWhatsapp($request, $is_attachment = false)
    {
        $return = [];
...
if (!function_exists('sendMessageChat')) {
    function sendMessageChat($request, $is_attachment = false)
    {
        $return = [];

        if (isset($request->displayName) && $request->displayName != "System Message") {
